...
export const createZeroTrustListsOneByOne = async (items) => {
  ...
  for (let i = 0, listNumber = 1; i < items.length; i += LIST_ITEM_SIZE) {
    const chunk = items
      ...
      .map((item) => ({ value: item }));
    const listName = `LLGP List - Chunk ${listNumber}`;
...
export const createZeroTrustRule = (wirefilterExpression) =>
  requestGateway("/rules", {
    method: "POST",
    body: JSON.stringify({
      name: "LLGP Filter Lists",
      description:
        "Filter lists created by Lilac Gateway Pi-hole.",
      enabled: true,
      action: "block",
      filters: ["dns"],
...
export const createZeroTrustRuleTest = (wirefilterExpression) =>
  requestGateway("/rules", {
    method: "POST",
    body: JSON.stringify({
      name: "LLGP Filter Lists",
